...
intersection = glob.glob('old_data/intersection/*')
...
data = all_data[20]
print(data)
# Séparation des données en colonnes
x_values = [item[1] for item in data]  # Deuxième colonne pour les abscisses
y_values = [item[0] for item in data]  # Première colonne pour les ordonnées

# Création du graphique
plt.figure(figsize=(8, 6))
plt.scatter(x_values, y_values, color='blue', label='Données')

# Définition des titres et labels des axes
plt.xlabel('participant')
plt.ylabel('zoom')
plt.xticks(range(int(min(x_values)), int(max(x_values)) + 1, 1))
